...
fn main() { // fn - function keyword, fn fn_name(){...}
    ...
    // This is an infinite loop, runs forever
    loop {
        ...
        io::stdin()
                  ...
                  .expect("Failed to read line"); // incase of invalid input crashes the program
        ...
        let guess: u32 = match guess.trim().parse() {
                                Ok(num) => num, /* The Ok arm checks whether the parse returns Ok, and it returns the value `num`.
                                                   The num value is then stored in the guess variable as u32 value.*/ 
                                Err(_) => continue,/* The Err arm checks for all the error states that the parse() method encounters
                                                      as is signified by the `_`. The `_` is a catch-all value */
                            };
        ...
        */ 
        match guess.cmp(&secret_number) {
            ...
            Ordering::Equal => { // The ordering variant can have multiple statements
                println!("You Win!");
                break; // exits the loop, similar to break in C
            }
        }
    }
}